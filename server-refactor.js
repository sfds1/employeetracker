...
async function addEmployee() {
   ...
   const roleTable = roles.map((role) => ({ name: role["First Name"], value: id }));

   console.log(roleTable);
   // Creating the employee list by concatenating the names to choose the manager
   const employees = await dbcalls.viewemp2()
   ...
   const answer = await askEmp(roleTable, employeeTable)

   const newEmp = await dbcalls.addemployee(answer)
   viewEmployees();
}
async function askEmp(roleTable, employeeTable) {
